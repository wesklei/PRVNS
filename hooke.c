...
#define      VARS		(255)	/* max # of variables	     */
...
double best_nearby(delta, point, prevbest, nvars, FUNCTION, aval_cont,itermax)/*{{{*/
	   ...
	   int 		    itermax;
{
	   ...
	   for (i = 0; i < nvars; i++) {
		   z[i] = point[i] + delta[i];
		   /* ftmp = f(z, nvars); */
		   ftmp = objfunc(z,&FUNCTION, &nvars,&cont);
		   if(itermax < cont){
			   minf=prevbest;
			   return (minf);
		   }
	   ...
	   *aval_cont = *aval_cont + cont;
	   return (minf);
}/*}}}*/

int hooke(nvars, startpt, endpt, rho, epsilon, itermax, FUNCTION,aval_cont)/*{{{*/
...

{
	   ...
	   fbefore = objfunc(startpt,&FUNCTION, &nvars,aval_cont);
	   newf = fbefore;
	   while ((iters < itermax) && (steplength > epsilon)) {
		   ...
		   newf = best_nearby(delta, newx, fbefore, nvars,FUNCTION,aval_cont,itermax-iters);
		   /* if we made some improvements, pursue that direction */
		   keep = 1;
		   iters=*aval_cont;
		   while ((newf < fbefore) && (keep == 1) && (iters < itermax)) {
			   /* iadj = 0; */
			   for (i = 0; i < nvars; i++) {
