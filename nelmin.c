...
void nelmin (  int n, double start[], double xmin[], 
		double *ynewlo, double reqmin, double step[],  int kcount, 
		int *icount, int *numres, int *ifault, int FUNCTION )
	...
	*/
{
	...
	*numres = 0;
	...
	for ( ; ; )
	{
		if((*icount + n) >= kcount) return;
		for ( i = 0; i < n; i++ )
		{ 
			p[i+n*n] = start[i];
		}
		/* y[n] = fn ( start ); */
	   	y[n] = objfunc(start,&FUNCTION, &n,icount);
		...
		for ( j = 0; j < n; j++ )
		{
			...
			/* y[j] = fn ( start ); */
	   		y[j] = objfunc(start,&FUNCTION, &n,icount);
			/* *icount = *icount + 1; */
			start[j] = x;
		}
		/*                 
		...
		for ( ; ; )
		{
			...
			*ynewlo = y[0];
			...
			/* ystar = fn ( pstar ); */
	   		ystar = objfunc(pstar,&FUNCTION, &n,icount);
			/* *icount = *icount + 1; */
			if(*icount >= kcount) return;
			/*
			...
			if ( ystar < ylo )
			{
				...
				/* y2star = fn ( p2star ); */
				y2star = objfunc(p2star,&FUNCTION, &n,icount);
				/* *icount = *icount + 1; */
				if(*icount >= kcount) return;
				/*
				   Check extension.
				...
				else if ( l == 0 )
				{
					...
					if ( y[ihi] < y2star )
					{
						for ( j = 0; j < nn; j++ )
						{
							...
							/* y[j] = fn ( xmin ); */
							y[j] = objfunc(xmin,&FUNCTION, &n,icount);
							/* *icount = *icount + 1; */
							if(*icount >= kcount) return;
						}
						ylo = y[0];
		...
		*ifault = 0;
		...
		if( (*icount + (2*n + 1)) >= kcount) return;
		for ( i = 0; i < n; i++ )
		{
			del = step[i] * eps;
			xmin[i] = xmin[i] + del;
			/* z = fn ( xmin ); */
			z = objfunc(xmin,&FUNCTION, &n,icount);
			/* *icount = *icount + 1; */
			if ( z < *ynewlo )
			{
				*ifault = 2;
